...
const IdentifierModal = forwardRef(function IdentifierModal({ user }, ref) {
  ...
  const newIdentifier = useSelector((state) => state.identifierModalSlice.item);
  ...
  const findItem = (type, id) => {
    if (type === "project") {
      const project = projects.data.find((project) => project.projectId === id);
      if (project) {
        setSelectedProject(project);
      }
    } else if (type === "page" && selectedProject) {
      const page = selectedProject.pages.find((page) => page.pageId === id);
      if (page) {
  ...
  return createPortal(
    <div>
      <dialog ref={modal} className="modal idf-modal">
              ...
              <ul className="idf-modal--ul">
                ...
                {projects.map((project) => (
                  <li
                    ...
                    onClick={() => findItem("project", project.projectId)}
                  >
                    <input
                      type="radio"
                    ...
                    />
                    <label htmlFor="project">{project.projectName}</label>
                  </li>
                ))}
              </ul>
            </section>
            <section className="idf-modal--page">
              {selectedProject && selectedProject.pages.length > 0 ? (
                <ul className="idf-modal--ul">
                  {/* page card */}
                  {selectedProject.pages.map((page) => (
                    <li
                      ...
                      className={`idf--page-card ${
                        selectedPage?.pageId === page.pageId ? `selected` : ""
                      }`}
                      onClick={() => findItem("page", page.pageId)}
                    >
                      <input
                        type="radio"
                        id={page.pageId}
                        ...
                        checked={selectedPage?.pageId === page?.pageId}
                        className="btn-radio"
                      />
                      <label htmlFor="page">{page.pageName}</label>
                    </li>
                  ))}
                </ul>
              ) : (
                <div className="idf-modal--no-project">No pages</div>
              )}
            </section>
                ...
                <div className="idf-modal--idf__content">
                  {type === "variable" ? (
                    <div className="idf-modal--idf__old">
                      <div className="idf-modal--idf__header">Variables</div>
                      {isLoading ? (
                        <img
                          src={Spinner}
                    ...
                    <ul className="idf-modal--ul idf-modal--idf__old">
                      {/* function card */}
                      <div className="idf-modal--idf__header">Functions</div>
                      {isLoading ? (
                        <img src={Spinner} className="loading-sub" />
                      ) : (
                        selectedPage.functions.map((fn) => (
                          <li key={`function-${fn.functionId}`}>
                            <ModalCard
        ...
        {user && projects && projects.length > 0 && selectedPage ? (
