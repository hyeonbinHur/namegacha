...
import * as aiAPI from "../../../utils/api/axios/aiApi";
import {
  setThread,
  pushAiMessages,
  pushUserMessages,
...
export default function ChatBox() {
  ...
  const checkStatus = async (runId) => {
    try {
      const responseStatus = await aiAPI.getStatus(currentThread, runId);
  ...
  const readReply = async (runId) => {
    try {
      ...
      if (status === "completed") {
        const messages = await aiAPI.getMessage(currentThread);
        dispatch(
          pushAiMessages({
            message: messages[0],
          })
  ...
  return (
    <div className="chat--box--container">
      ...
      <div className="chat--box__buttons">
        ...
        {isLoading && <img src={Spinner} className="chat--box__loading" />}
        {userMessage.length > 0 ? (
          ...
          <button className="chat--box__button--send__inactive">Send</button>
        )}
      </div>
    </div>
  );
